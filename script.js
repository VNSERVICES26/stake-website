...
async function stakeTokens() {
    ...
    try {
        ...
        if (!referrer || !web3.utils.isAddress(referrer)) {
            // अगर रेफरर नहीं है, तो कॉन्ट्रैक्ट ओनर को डिफॉल्ट रेफरर बनाएं
        ...
        // Skip referral stake check if admin is creating first stake
        if (!isAdmin) {
            // रिफरर वैलिडेशन - सामान्य यूजर्स के लिए
            const referrerStake = await vnstStakingContract.methods.stakes(referrer).call();
            if (!referrerStake.active || referrerStake.amount < web3.utils.toWei("100", "ether")) {
                showError("Referrer must have an active stake of at least 100 VNST");
                hideLoading('stakeBtn');
                return;
            }
        }
...
async function createFirstStake() {
    try {
        ...
        if (parseInt(allowance) < parseInt(stakeAmount)) {
            ...
            // अप्रूवल कन्फर्म होने का इंतजार करें
            await new Promise(resolve => setTimeout(resolve, 10000));
        }
...
async function initApp() {
    try {
        ...
        // Hide data sections if they exist
